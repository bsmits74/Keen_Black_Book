...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{imgs/drawings/angle_ratio_new.eps}
\caption{Simplify to 22$^{\circ}$ for xspeed > 2x yspeed.}
\label{fig:angles}
\end{figure}

\par
To simplify calculations, the game restricts incoming angles to four discrete values -- 22$^{\circ}$, 45$^{\circ}$, 67$^{\circ}$ or 90$^{\circ}$ -- based on the \cw{xspeed} to \cw{yspeed} ratio. Eight angle indices are then defined,where indices 0–3 represent negative \cw{xspeed}, while indices 4–7 represent positive \cw{xspeed}.\\
...
\vspace{10pt}
\par
For each combination of the eight slope types and angle index, the bounce angle is determined using a simple lookup table. \\
...
\par
Each entry in the table corresponds to one of 16 possible bounce angles. For example, when a wall type 3 slope is hit with an incoming angle of 22$^{\circ}$ (angle index 0) and postive \cw{xspeed}, the lookup table refers to bounce angle \#5. Each bounce angle is decomposed into a new \cw{xspeed} and \cw{yspeed}.\\
...
\par
It is worth noting that in some cases, the resulting bounce angle does not follow the laws of physics. For instance, an incoming angle of 22$^{\circ}$ (incoming angle index 0) on a 45$^{\circ}$ slope (walltype index 4) results in a bounce angle of 90$^{\circ}$ instead of the expected 67$^{\circ}$.\\
