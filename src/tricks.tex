...
\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{imgs/drawings/angle_0.eps}
\label{fig:angles}
\end{figure}
\par
...
When a new level is loaded, the screen fades from black to the default colors. CHECK WHEN SCREEN FADES TO WHITE.
Here it makes use of reassigning the color palette. This can easily be done by calling BIOS software interrupt \cw{10h}.\\
...
\hline
\textbf{\color{black} Pin} & \textbf{\color{black} EGA modes (rgbRGB)} & \textbf{\color{black} CGA modes (IRGB)} \\
\hline
...
Fading the screen from black to color is rather straight forward.\\
\par
\begin{minipage}{\textwidth}
