...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{imgs/drawings/angle_ratio_new.eps}
\caption{Simplify to 22$^{\circ}$ for xspeed more than twice the yspeed.}
\label{fig:angles}
\end{figure}

\par
To simplify the calculations, the game restricts incoming angles to four discrete values -- 22$^{\circ}$, 45$^{\circ}$, 67$^{\circ}$ or 90$^{\circ}$ -- based on the ratio of \cw{xspeed} to \cw{yspeed}. Eight angle indices are then defined, where indices 0–3 represent negative \cw{xspeed}, while indices 4–7 represent positive \cw{xspeed}. The resulting speed vector magnitude is calculated using either \cw{xspeed} or \cw{yspeed}, depending on which has the greater absolute value. This magnitude is multiplied by 256 to achieve higher precision.\\
...
\begin{tabular}{
 ...
 & \multicolumn{8}{c}{\textbf{incoming angle index}} \\ 
...
\par
Each entry in the table corresponds to one of 16 possible bounce angles. For example, when a wall type 3 slope is hit with an incoming angle of 22$^{\circ}$ (angle index 0) and positive \cw{xspeed}, the lookup table specifies bounce angle \#5. Each bounce angle is then decomposed into a new \cw{xspeed} and \cw{yspeed}.\\


\par
\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{imgs/drawings/bounce_angle.eps}
...
\par
It is worth noting that in some cases, the resulting bounce angle does not follow the laws of physics. For instance, an incoming angle of 22$^{\circ}$ (angle index 0) on a 45$^{\circ}$ slope (wall type 4) results in a bounce angle of 90$^{\circ}$, rather than the expected 67$^{\circ}$.\\


\par
\begin{figure}[H]
\centering
\includegraphics[width=0.38\textwidth]{imgs/drawings/bounce_physics.eps}
...
\par
In total, six pre-defined color palettes are stored in the \cw{color[][]} table, ranging from completely black to completely white. The default table with CGA colors is defined in row 3.\\
...
\end{tabularx}\\
\setlength{\tabcolsep}{6pt} % reset border margin
\caption{Color palette table \cw{colors[][]}.}
\end{figure}
