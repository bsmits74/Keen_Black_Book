...
\par
The CGA card is also making use of memory mapping, just like EGA. In mode \cw{04h}, the VRAM bank 0 is mapped from 0xB0000 to 0xB1FFF and VRAM bank 1 is mapped from 0xB2000 to 0xB3FFF. Unlike EGA, the CGA memory model doesn't require masking as the total 16KiB VRAM fits easy in a 64KiB memory segment.\\ 
...
A full picture in mode \cw{04h} requires 320 pixels * 2 bits per pixel * 200 lines = 16,000 bytes of memory. So the main screen requires all 16KiB memory and there is no capacity in VRAM for extra screens.   \\
