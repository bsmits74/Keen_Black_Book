...
\label{state_type}
\par
This memory buffer contains both the video buffer page and master page. The buffer page starts at offset \cw{0000h}, while the master page begins at \cw{8000h}. Both pages float within the 64 KiB memory segment, leveraging the same memory wrapping mechanism described in section "\nameref{section:wrap_ega_memory}" on page \pageref{section:wrap_ega_memory}.\\
...
\par
The CPU lacked the speed to copy the entire RAM buffer to VRAM during the vertical retrace period, which was the only time the CGA hardware did not output to the screen. Consequently, screen tearing in the CGA version of Commander Keen was unavoidable.\\

\trivia{Because the video memory on the original IBM CGA was not dual-ported, when both the CPU and the video output attempted to access the same byte of video RAM, the CPU took precedence. As a result, the card ends up reading a random value, causing white snow on the screen.}
