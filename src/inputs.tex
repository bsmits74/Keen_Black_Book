...
\par
This encoding also has the additional benefit that, if the underlying software only understands the 84-key layout, it will (presumably) discard the \cw{E0h} byte in this example and only process the \cw{35h} byte. This would result in a "/" being correctly typed from the numeric keypad, even though the software doesn't understand that this key exists. The picture below on this and previous page shows the full IBM PS/2 keyboard scancode layout.\\
