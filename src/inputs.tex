...
\vfill
      \includegraphics[width=1.0\textwidth]{imgs/drawings/keyboard_model_M_left.eps}
...
\trivia{The \cw{E0h} scancode introduced some unexpected behaviors. For example, typing "Shift" + "Num /" should produce a "/", as indicated on the keycap. However, older software may interpret this sequence incorrectly: \textit{Shift is down, I don't understand \cw{E0h}, and here comes \cw{35h}. That means the user wanted to type "?".}}\\

\par
The keys "Print Screen/SysRq" and "Pause/Break" are special cases. The former produces scancode "\cw{e0h,2ah,e0h,37h}", together with Shift or Ctrl it produces "\cw{e0h,37h}", and \cw{54h} together with (left or right) Alt. The latter produces scancode sequence \linebreak "\cw{e1h,1dh,45h,e1h,9dh,c5h}" when pressed and nothing when released. Together with (left or right) Ctrl, one gets "\cw{e0h,46h,e0h,c6h}", and again nothing at release. The picture below on this and previous page shows the full IBM PS/2 keyboard scancode layout.\\


\par
The Input Manager maintains several lookup tables to translate the scancodes to ASCII or special codes
\begin{itemize}
	\item Shifted and unshifted ASCII codes tables.
	\item Table for the 14 duplicate keys with \cw{E0h} prefix.
	\item Special keys like "ESC", "Shift", "Backspace", etc.
\end{itemize}

\vfill
      \includegraphics[width=1.0\textwidth]{imgs/drawings/keyboard_model_M_right.eps}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{imgs/drawings/keyboard.eps}
