...
\begin{itemize}
  ...
  \item \cw{updatestart[]} points to the tile array buffer. It maintains which tiles needs to be updated upon next refresh. Here we have two buffer arrays, one for the screenpage and one for otherpage.
\end{itemize}
...
\begin{minipage}{.4\textwidth}
Point the visible screen into the buffer screen by updating the CRTR start address and horizontal Pel Panning. The entire tile buffer array is cleared to '0'. Finally the buffer and screenpage of both the \cw{screenstart[]} and \cw{update[]} are swapped. Then step 1 is repeated. \\
\par
