...
\par
In the best case, the engine must read and write 128 bytes (2 * 16 bytes * 4 memory banks) to VRAM for a background tile only. In the worst case, reading and writing up to 608 bytes are needed (background, foreground, sprite, and foreground re-drawn), with several bitwise operations involved. It must run quickly and therefore most of the code is written in assembly.
