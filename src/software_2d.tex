...
All objects in the game, such as Commander Keen, enemies, bonus points, doors, and even the scoreboard, are called "actors". Each actor can "think" and perform actions like walking, attacking, or playing sounds. Actors are controlled via a state machine, enabling them to take actions such as chasing the player, throwing objects, or jumping over a gap. To model their behavior, each enemy has an associated state, which can include:
\begin{itemize}
  \item Chasing Keen
...
The 286 CPU can read and write 16 bits in a single cycle, but there is a caveat: this only works when accessing even memory addresses. To make things worse, writing a word at offset \cw{FFFFh} causes the CPU to crash.\\
...
\par
Take for example drawing a 6-byte wide sprite. An optimal write depends on whether the start address is even or odd. In case of an even address, three times a \cw{word} writes is optimal. In case of an odd address, the optimal approach is to first write a \cw{byte}, followed by two \cw{word} writes and then a \cw{byte} again.\\
