...
\begin{itemize}
  \item \cw{*screenstart[]} are pointers to the starting address (upper-left pixel) of the viewports in VRAM. As explained, we maintain three viewports in VRAM:
  \begin{itemize}
  ...
  \end{itemize}
  \item \cw{*updatestart[]} are pointers to the tile arrays. It maintains which tiles needs to be updated upon next refresh. There are two tile buffer arrays; one for the screenpage and one for otherpage. 
\end{itemize}
...
\begin{enumerate}
\item Check if the player has moved one tile in any direction.
\item In case the player moved one tile, move the \cw{*screenstart[]} and \cw{*update[]} pointers accordingly. 
