...
For background tile animation two information tables are defined: \textbf{tile animation} and \textbf{tile animation speed}. The tile animation refers which tile, relative to this tile, the tile will animate to. In Table \ref{table:background tile anim} the next animation after tile \#90 is \#91 (+1), followed by \#92 (+1) and \#93 (+1). After tile \#93 (-3) the sequence is going back to tile \#90. The animation speed is expressed in \cw{TimeCount}, which is the number of ticks before the next tile is displayed. \\
 \begin{table}[H]
