...
\par
There was no need to enter command-line mode however. The IDE allowed to create a project, build, run and debug. The IDE allowed to create a project, build, run and debug. It contained a "high resolution" 50x80 text mode, which doubled the vertical resolution.\\
\par
...
\par
Foreground tiles, or "masked tiles", are similar to normal tiles but contain five planes instead of four. The extra plane stores the mask; the plane order is mask, blue, green, red, and intensity. Consequently, a single masked tile requires 160 bytes (128 + 32) of storage. A mask bit of '0' means the background tile color is erased and replaced by the foreground tile color, whereas a mask bit of '1' preserves the background color behind the foreground. If the foreground color is '0', the background color remains visible. \\
...
A map is created by placing tiles on each of these three layers. Foreground and background tiles can be enhanced with additional properties ("Tile info"), that controls interactions such as clipping, "deadly" tiles, and animated tiles. 
...
\par
All sound effects are done by Robert Prince. Sound effects are created using Cakewalk and inhouse MUSE tool\footnote{See \textit{Game Engine Black Book - Wolfenstein 3D}, section 3.6 for a extensive description.}.
...
On December 14th, 1990 the first episode was released via Apogee. Episodes 1-5 are all published by Apogee Software. The game engine and first episode were given for free and encouraged to be copied and distributed to a maximum number of people. To receive the other episodes, each player had to pay \$30 (for Episode 1-3) to \textit{ideas from the Deep}.\\
