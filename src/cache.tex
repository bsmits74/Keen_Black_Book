...
\caption{Initiating \cw{grneeded[]} array.}
\end{figure}
...
\par
When the user closes the control panel, the engine lowers the cache level back to one, where all assets required for playing the level are memorized. The function \cw{CA\_CacheMarks()} is called again to reload any assets that were removed from memory.\\
