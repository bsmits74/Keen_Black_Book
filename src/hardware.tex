...
\subsection{Overview}
  ...
  \par
  In 1982 Intel released the 80286 microchip. A typical 8088 chip was running at 4.77Mhz, where the 80286 was running at 8Mhz and later at 12.5Mhz. The 80286 was employed for the IBM PC/AT, introduced in 1984, and then widely used in most PC/AT compatible computers until the early 1990s. Commander Keen could run on a 8088, but an Intel 286 was recommended.\\   
...
\centering
  \begin{tikzpicture}[font=\small]
    \begin{axis}[
      ...
      symbolic x coords={8088 4.8Mhz, 286 6MHz,286 8MHz,386SX 16,386SX 33,386DX 33},
...
Although EGA supported high resolutions like 640x350 pixels, it required an expensive high resolution EGA monitor. For reasons of the compatibility with CGA and avoid acquiring an expensive EGA monitor most game programmers used  mode \cw{10h}, using the 320x200 resolution with 16 colors.
...
\par
Note that creating 320x200 picture on the screen requires 32KiB of VRAM (4 planes, each 8KiB). 
...
\par
 In practice the effective bandwidth of the bus is divided by two due to packet overhead and interrupts. As a result, a PC equipped with an 8 bit ISA EGA card can push 19.1Mbits/s/2/8 = 1.1MB/s. In mode \cw{0Dh}, since a frame is 320x200/2 = 32,000 bytes, the theoretical maximum framerate with a CPU taking 0ms to render a frame is 1,100,000 / 32,000 = 34 frames per second.\\
 \par
