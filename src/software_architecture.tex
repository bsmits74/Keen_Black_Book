...
\par
Bitshift tables reference are made globally available for all assembly routines via the \linebreak \cw{shifttabletable} variable.\\
\par
\begin{minipage}{\textwidth}
  \lstinputlisting[language={[x86masm]Assembler}]{code/unrolles_bitshift_tables.asm}
\end{minipage}\\

\par
The assembly routine \cw{ShiftPropChar} takes care of shifting and drawing the character to the \cw{(x,y)} location on screen.\\
...
\bigskip
The macro \cw{SHIFTNOXOR} prints the bitshifted character into the buffer.\\
\par
\begin{minipage}{\textwidth}
  \lstinputlisting[language={[x86masm]Assembler}]{code/unrolled_byte_shift_font.asm}
\end{minipage}\\

\par
The final step is copying the buffer to VRAM. Printing the letter "N" with an offset of 3 pixels involves only a simple lookup in the \cw{shiftdata3} table. By sacrificing 4096 bytes of memory, a lot of CPU time is saved.\\
\begin{figure}[H]
\centering
