...
\item The \cw{RF\_*} layer is used to refresh tiles, and is also made of both C and ASM code.
\end{itemize}
...
To manage and keep track of the assets to be loaded into memory, an array \cw{gr\_needed[]} is maintained to mark if an asset needs to be loaded from disk. The index of this array refers to the enum of the graphic assets. By using the eight bits the array can maintain the required assets for different levels.
