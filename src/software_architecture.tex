...
\par
The game was distributed on a single 5\nicefrac{1}{4}-inch, 360 KiB floppy disk. The compiled executable alone already occupies 349 KiB, and it still needed to accommodate graphics, level, and audio assets. With only 11 KiB remaining, the team needed a way to compress the executable to ensure everything would fit on one floppy disk.\\
...
\par
Bitshift tables are arranged after each other and made globally available for all assembly routines.\\
\par
\begin{minipage}{\textwidth}
...
\bigskip
The macro \cw{SHIFTNOXOR} prints table shifts the character into the buffer.\\
\par
\begin{minipage}{\textwidth}
...
\par
The final step is copying the buffer to VRAM. Printing the letter "N" with an offset of 3 pixels involves a simple lookup in the \cw{shiftdata3} table. By sacrificing 4096 bytes of memory, a lot of CPU time is saved.\\
\begin{figure}[H]
\centering
