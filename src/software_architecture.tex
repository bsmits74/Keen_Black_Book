...
\trivia{At time of writing the game Red Dead Redemption II is estimated to have the highest lines of code count: 60 million lines of code.}\\
...
\par
The final step involved copying the buffer to VRAM. Printing the letter "N" with an offset of 3 pixels involves a simple lookup in the \cw{shiftdata3} table. By sacrificing 4096 bytes of memory, a lot of CPU time is saved.\\
\begin{figure}[H]
\centering
...
The input manager abstracts interactions with joystick, keyboard, and mouse. It features the code to deal with PS/2, Serial, and DA-15 ports, with each using their own I/O addresses. The keyboard is descibed in the "\nameref{keyboard}" section on page \pageref{keyboard}.\\
