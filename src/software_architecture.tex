...
The Cache Manager is a small but critical component. It loads and decompresses maps, graphics and audio resources stored on the filesystem and makes them available in RAM. Assets are stored in three files: 
\begin{itemize}
...
On the EGA video card each 8 pixels take up 1 byte of VRAM, as explained on page \pageref{section:EGA_Planar_Madness}. When you print characters that aren't perfectly aligned with this 8-pixel grid, how do you manage the alignment? Let's say you want to display the letter 'N' on the screen. If the 'N' starts in the middle of a byte (say at pixel 3 instead of pixel 0), you need a clever way to shift the bits over to ensure it appears correctly on the screen.\\
...
\par
Because most players used the keyboard to play Commander Keen, section “\nameref{keyboard}” on page \pageref{keyboard} provides a detailed explanation of the keyboard input system and its implementation.\\
